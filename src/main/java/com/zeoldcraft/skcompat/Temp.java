			...
			try {
				...
				if (cre.getExceptionType().equals(ExceptionType.LengthException)) {
					throw new ConfigRuntimeException("The given string was the wrong size to identify a player."
													 + " A player name is expected to be between 1 and 16 characters. "
													 + cre.getMessage(),
							ExceptionType.LengthException, t);
				} else {
					throw cre;
...
//throw a CRE instead.
		...
		if (m == null || (m instanceof MCPlayer && (!((MCPlayer) m).isOnline()
													&& !injectedPlayers.containsKey(player)))) {
			throw new ConfigRuntimeException(
					"The specified player (" + player + ") is not online", ExceptionType.PlayerOfflineException, t);
		}
		return m;
	}
